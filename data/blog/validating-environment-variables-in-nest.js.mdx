...
However, when we now run `docker compose up` we can see that we can't connect to our database, because our `.env` file is empty.
...
When you now start up the development server with `docker compose up` again and still have an empty `.env` file, you see a more detailed output than before:
...
When we now start everything up again with `docker compose up`, our application should start properly without any errors:
...
When we now run `docker compose up`, we see the following:
...
Running `docker compose up` again, we don't see any errors and our application starts up properly:
